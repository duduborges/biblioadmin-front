...
export default function CadastroLivro() {
    ...
    const fetchLivros = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://189.8.205.53:8010/biblio/livro');
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (add) {
                await axios.post('https://189.8.205.53:8010/biblio/livro', novoLivro);
            } else {
                await axios.put("https://189.8.205.53:8010/biblio/livro", novoLivro,);
            }
            fetchLivros();
            onCloseModal();
            setIsUpdated(false)
    ...
    const handleDelete = async (id: number) => {
        try {
            await axios.delete(`https://189.8.205.53:8010/biblio/livro/id/${id}`);
            fetchLivros();
            setIsDeleted(false)
        } catch (error) {
